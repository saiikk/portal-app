...
export default function ProfileScreen() {
  ...
  const closeAvatarPreview = () => {
    Animated.timing(scaleAnim, {
      ...
      duration: 150,
      useNativeDriver: true,
    }).start(() => setShowAvatarPreview(false));
  };

  const { mutate: saveProfile, isPending } = useMutation({
    mutationFn: () =>
      profileApi.updateProfile({ name: name.trim(), email: email.trim() }),
    onSuccess: (updated) => {
      setUser(updated);
      setIsEditing(false);
      Alert.alert("保存しました");
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{ padding: s(24) }}
    >
      {/* アバター */}
      ...
      {/* 名前 */}
      <Text style={{ fontSize: s(13), color: "#666", marginBottom: s(6) }}>
        名前
      </Text>
      <TextInput
        style={{
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text style={{ color: "#666", fontSize: s(15), fontWeight: "600" }}>
          ログアウト
        </Text>
      </TouchableOpacity>
      ...
      <Modal visible={showPasswordModal} transparent animationType="fade">
        ...
        >
          ...
          >
            ...
            <Text
              style={{ fontSize: s(16), fontWeight: "600", marginBottom: s(8) }}
            >
              メールを送信しました
            </Text>
            <Text
            ...
            >
              <Text
                style={{ color: "#fff", fontWeight: "600", fontSize: s(14) }}
              >
                OK
              </Text>
            </TouchableOpacity>
